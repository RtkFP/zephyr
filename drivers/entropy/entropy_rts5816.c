...
#define DT_DRV_COMPAT realtek_rts5816_trng
...
#include <soc.h>
#include <string.h>
#include "entropy_rts5816.h"
#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(entropy_rts5816, CONFIG_ENTROPY_LOG_LEVEL);

struct trng_rts5816_cfg {
	mem_addr_t regs;
};

static int entropy_rts5816_get_entropy(const struct device *dev, uint8_t *buffer, uint16_t length)
{
	const struct trng_rts5816_cfg *cfg = dev->config;
...
static int entropy_rts5816_init(const struct device *dev)
{
	return 0;
}

static const struct entropy_driver_api entropy_rts5816_api = {
	.get_entropy = entropy_rts5816_get_entropy, .get_entropy_isr = NULL};

static const struct trng_rts5816_cfg trng_rts5816_cfg = {
	.regs = (mem_addr_t)DT_INST_REG_ADDR(0),
};

DEVICE_DT_INST_DEFINE(0, entropy_rts5816_init, NULL, NULL, &trng_rts5816_cfg, PRE_KERNEL_1,
		      CONFIG_ENTROPY_INIT_PRIORITY, &entropy_rts5816_api);
