...
#include "flash_rts5816_qspi_reg.h"
#include "flash_rts5816_qspi.h"

#define DT_DRV_COMPAT realtek_rts5816_qspi

LOG_MODULE_REGISTER(flash_rts5816_qspi, CONFIG_FLASH_LOG_LEVEL);

#define CACHE_SPI_AUTOMODE_TRANSFER_MODE_OFFSET 8
#define CACHE_A_SPI_DUM_BIT_NUM_OFFSET          16
#define ERASE_SECTOR_SIZE                       0x1000

struct flash_rts5816_qspi_config {
	mem_addr_t base;
	mem_addr_t cs_reg;
	mem_addr_t cmd_reg;
...
struct flash_rts5816_qspi_data {
...
{
	const struct flash_rts5816_qspi_config *dev_conf = dev->config;
...
{
	struct flash_rts5816_qspi_data *dev_data = dev->data;
...
static int flash_rts5816_qspi_read(const struct device *dev, off_t addr, void *data, size_t size)
{
...
static int flash_rts5816_qspi_write(const struct device *dev, off_t offset, const void *data,
				    size_t len)
{
	struct flash_rts5816_qspi_data *dev_data = dev->data;
...
static int flash_rts5816_qspi_erase(const struct device *dev, off_t offset, size_t size)
{
	struct flash_rts5816_qspi_data *dev_data = dev->data;
	uint32_t addr = (uint32_t)offset;
	int ret;
...
static const struct flash_parameters flash_rts5816_qspi_parameters = {.write_block_size = 1,
								      .erase_value = 0xff};

static const struct flash_parameters *flash_rts5816_qspi_get_parameters(const struct device *dev)
{
	ARG_UNUSED(dev);

	return &flash_rts5816_qspi_parameters;
}

#if defined(CONFIG_FLASH_PAGE_LAYOUT)
static void flash_rts5816_qspi_pages_layout(const struct device *dev,
...
static const struct flash_driver_api flash_rts5816_qspi_driver_api = {
	.read = flash_rts5816_qspi_read,
	.write = flash_rts5816_qspi_write,
	.erase = flash_rts5816_qspi_erase,
	.get_parameters = flash_rts5816_qspi_get_parameters,
#if defined(CONFIG_FLASH_PAGE_LAYOUT)
	.page_layout = flash_rts5816_qspi_pages_layout,
#endif
};

static int flash_rts5816_qspi_init(const struct device *dev)
{
	const struct flash_rts5816_qspi_config *dev_cfg = dev->config;
	struct flash_rts5816_qspi_data *dev_data = dev->data;
	const spi_flash_params *sf_table = NULL;
	clock_control_subsys_rate_t rate = (clock_control_subsys_rate_t)60000000;
	uint32_t id_read;
	uint8_t i;
	int ret;
...
static const struct flash_rts5816_qspi_config flash_rts5816_qspi_cfg = {
...
static struct flash_rts5816_qspi_data flash_rts5816_qspi_dev_data;

DEVICE_DT_INST_DEFINE(0, &flash_rts5816_qspi_init, NULL, &flash_rts5816_qspi_dev_data,
		      &flash_rts5816_qspi_cfg, POST_KERNEL, CONFIG_FLASH_INIT_PRIORITY,
		      &flash_rts5816_qspi_driver_api);
