...
#include <zephyr/dt-bindings/gpio/rts5816-gpio.h>
#include "../pinctrl/pinctrl_rts5816.h"

#define DT_DRV_COMPAT realtek_rts5816_gpio
...
static int rts_fp_pin_configure(const struct device *port, gpio_pin_t pin, gpio_flags_t flags)
{
	...
	if (flags & GPIO_OUTPUT) {
		...
		if ((flags & GPIO_PULL_DOWN) != 0) {
			if (pin < GPIO_AL0) {
				rts_writel_mask(base + PAD_CFG + pin * PAD_CFG_SIZE, PD,
						PU_MASK | PD_MASK);
			} else if (pin >= GPIO_AL0 && pin <= GPIO_AL2) {
				rts_writel_mask(
				...
				rts_writel_mask(sp_base0 + AL_SENSOR_SCS_N_CTL, SENSOR_SCS_N_PDE,
			...
			if (pin < GPIO_AL0) {
				rts_writel_mask(base + PAD_CFG + pin * PAD_CFG_SIZE, PU,
						PU_MASK | PD_MASK);
			} else if (pin >= GPIO_AL0 && pin <= GPIO_AL2) {
				rts_writel_mask(
					sp_base0 + AL_GPIO_CFG,
				...
				rts_writel_mask(sp_base0 + AL_SENSOR_RST_CTL, SENSOR_RSTN_PUE,
						SENSOR_RSTN_PUE_MASK | SENSOR_RSTN_PDE_MASK);
			} else if (pin == GPIO_SNR_CS) {
				rts_writel_mask(sp_base0 + AL_SENSOR_SCS_N_CTL, SENSOR_SCS_N_PUE,
						SENSOR_SCS_N_PUE_MASK | SENSOR_SCS_N_PDE_MASK);
			} else if (pin == GPIO_SNR_GPIO) {
				rts_writel_mask(sp_base0 + GPIO_SVIO_CFG,
	...
	if (flags & GPIO_POWER_3V3) {
		if (pin < GPIO_AL0) {
			rts_writel_mask(base + PAD_CFG + pin * PAD_CFG_SIZE, H3L1,
					H3L1_MASK | IEV18_MASK);
		} else if (pin == GPIO_SNR_RST) {
			rts_writel_mask(sp_base0 + AL_SENSOR_RST_CTL, SENSOR_RSTN_H3L1,
					SENSOR_RSTN_IEV18_MASK | SENSOR_RSTN_H3L1_MASK);
		} else if (pin == GPIO_SNR_CS) {
			rts_writel_mask(sp_base0 + AL_SENSOR_SCS_N_CTL, SENSOR_SCS_N_H3L1,
					SENSOR_SCS_N_IEV18_MASK | SENSOR_SCS_N_H3L1_MASK);
		} else if (pin == GPIO_SNR_GPIO) {
			rts_writel_mask(sp_base0 + GPIO_SVIO_CFG, GPIO_SVIO_H3L1,
				   ...
				   enum gpio_int_mode mode, enum gpio_int_trig trig)
{
	...
	if (mode == GPIO_INT_MODE_DISABLED) {
		...
		if (pin == GPIO_CACHE_CS2) {
			...
			sys_write32(GPIO_FALLING_INT | GPIO_RISING_INT,
				    base + PAD_GPIO_INT + pin * PAD_CFG_SIZE);

			if (trig == GPIO_INT_TRIG_LOW) {
				sys_write32(GPIO_FALLING_INT,
					    base + PAD_GPIO_INT_EN + pin * PAD_CFG_SIZE);
			} else if (trig == GPIO_INT_TRIG_HIGH) {
				sys_write32(GPIO_RISING_INT,
					    base + PAD_GPIO_INT_EN + pin * PAD_CFG_SIZE);
			...
			sys_write32(SENSOR_RSTN_GPIO_INT_FALL | SENSOR_RSTN_GPIO_INT_RISE,
			...
			if (trig == GPIO_INT_TRIG_LOW) {
				...
				sys_set_bits(sp_base0 + AL_SENSOR_RST_CTL,
					     SENSOR_RSTN_GPIO_INT_EN_FALL |
						     SENSOR_RSTN_GPIO_INT_EN_RISE);
			}
		} else if (pin == GPIO_SNR_CS) {
			sys_write32(SENSOR_SCS_N_GPIO_INT_FALL | SENSOR_SCS_N_GPIO_INT_RISE,
				    sp_base1 + SENSOR_INT_FLAG);
			...
			if (trig == GPIO_INT_TRIG_LOW) {
				...
				sys_set_bits(sp_base0 + AL_SENSOR_SCS_N_CTL,
					     SENSOR_SCS_N_GPIO_INT_EN_FALL |
						     SENSOR_SCS_N_GPIO_INT_EN_RISE);
			}
		} else if (pin == GPIO_SNR_GPIO) {
			sys_write32(GPIO_SVIO_INT_FALL | GPIO_SVIO_INT_RISE,
				    sp_base1 + SENSOR_INT_FLAG);
			...
			if (trig == GPIO_INT_TRIG_LOW) {
				...
				sys_set_bits(sp_base0 + GPIO_SVIO_CFG,
					     GPIO_SVIO_INT_EN_FALL | GPIO_SVIO_INT_EN_RISE);
			}
		} else if (pin >= GPI_WAKE1 && pin <= GPI_WAKE2) {
...
static int rts_fp_sp_gpio_isr(const struct device *port)
{
	...
	for (int i = 0; i < config->ngpios; i++) {
		if (i >= GPIO_AL0 && i <= GPIO_AL2) {
			...
			if (int_rising_flag && int_rising_en) {
				sys_write32(SENSOR_RSTN_GPIO_INT_RISE, sp_base1 + SENSOR_INT_FLAG);
				pins |= BIT(i);
			} else if (int_falling_flag && int_falling_en) {
				sys_write32(SENSOR_RSTN_GPIO_INT_FALL, sp_base1 + SENSOR_INT_FLAG);
				pins |= BIT(i);
			}
		} else if (i == GPIO_SNR_CS) {
			int_rising_flag = sys_read32(sp_base1 + SENSOR_INT_FLAG) &
			...
			if (int_rising_flag && int_rising_en) {
				sys_write32(SENSOR_SCS_N_GPIO_INT_RISE, sp_base1 + SENSOR_INT_FLAG);
				pins |= BIT(i);
			} else if (int_falling_flag && int_falling_en) {
				sys_write32(SENSOR_SCS_N_GPIO_INT_FALL, sp_base1 + SENSOR_INT_FLAG);
				pins |= BIT(i);
			}
		} else if (i == GPIO_SNR_GPIO) {
			int_rising_flag =
			...
			if (int_rising_flag && int_rising_en) {
				sys_write32(GPIO_SVIO_INT_RISE, sp_base1 + SENSOR_INT_FLAG);
				pins |= BIT(i);
			} else if (int_falling_flag && int_falling_en) {
				sys_write32(GPIO_SVIO_INT_FALL, sp_base1 + SENSOR_INT_FLAG);
				pins |= BIT(i);
			}
		} else if (i >= GPI_WAKE1 && i <= GPI_WAKE2) {
			k_busy_wait(30); /* wake1&wake2 have 20us deglitch */
			int_rising_flag = sys_read32(sp_base1 + SENSOR_INT_FLAG) &
...
static const struct gpio_driver_api rts_fp_gpio_api = {
...
static int rts_fp_pins_init(const struct device *dev)
{
	 ...
	 * gpios. rts5816 has cache_cs2 and sensor_cs two pins which default function is not gpio
	 */
	sys_write32(0x1, base + PAD_GPIO_INC);
	rts_writel_mask(base + PAD_CFG, BIT(9), PAD_FUNCTION_SEL_MASK);
	rts_writel_mask(sp_base0 + AL_SENSOR_SCS_N_CTL, SENSOR_SCS_N_SEL, SENSOR_SCS_N_SEL_MASK);
