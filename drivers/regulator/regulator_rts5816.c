...
#include "regulator_rts5816.h"

LOG_MODULE_REGISTER(regulator_rts5816, CONFIG_REGULATOR_LOG_LEVEL);

#define DT_DRV_COMPAT realtek_rts5816_regulator
...
static int regulator_rtsfp_enable(const struct device *dev)
{
	...
	if (id == LDO_SVIO) {
		if (config->ocp_en) {
			sys_set_bits(base + R_LDO_TOP_OC_SVIO, OC_EN_SVIO | OC_POW_OFF_EN_SVIO);
			sys_set_bits(R_AL_DUMMY1, SVIO_SUSPEND_OCP_EN);
		} else {
			sys_clear_bits(base + R_LDO_TOP_OC_SVIO, OC_EN_SVIO | OC_POW_OFF_EN_SVIO);
		...
		if (config->ocp_en) {
			sys_set_bits(base + R_LDO_TOP_OC_SVA, OC_EN_SVA | OC_POW_OFF_EN_SVA);
			sys_set_bits(R_AL_DUMMY1, SVA_SUSPEND_OCP_EN);
		} else {
			sys_clear_bits(base + R_LDO_TOP_OC_SVA, OC_EN_SVA | OC_POW_OFF_EN_SVA);
...
static int puf_otp_read(const struct regulator_rtsfp_config *cfg, _off_t offset, void *data,
			size_t len)
{
...
static int rts5816_regulator_resume(const struct device *dev)
{
	const struct regulator_rtsfp_config *config = dev->config;
	mem_addr_t base = config->base;
...
static int rts5816_regulator_suspend(const struct device *dev)
{
	const struct regulator_rtsfp_config *config = dev->config;
	mem_addr_t base = config->base;
...
static int rts5816_regulator_pm_action(const struct device *dev, enum pm_device_action action)
{
	int ret;

	switch (action) {
	case PM_DEVICE_ACTION_RESUME:
		ret = rts5816_regulator_resume(dev);
		break;
	case PM_DEVICE_ACTION_SUSPEND:
		ret = rts5816_regulator_suspend(dev);
		break;
	default:
		return -ENOTSUP;
	}
...
static int puf_otp_init(const struct device *dev)
{
	...
	rts_writel_mask(base + R_LDO_TOP_POW, POW_PWD_PUFF | POW_LED,
			POW_PWD_PUFF_MASK | POW_LED_MASK);
	k_busy_wait(100);

	rts_writel_mask(base + R_LDO_TOP_POW, PUFF_ISO, PUFF_ISO_MASK);
	k_busy_wait(10);
...
#define REGULATOR_RTSFP_DEFINE(node, id)                                                           \
	...
	PM_DEVICE_DT_INST_DEFINE(node, rts5816_regulator_pm_action);                               \
	DEVICE_DT_DEFINE(node, regulator_rtsfp_init, PM_DEVICE_DT_INST_GET(node), &data_##id,      \
			 &config_##id, POST_KERNEL, CONFIG_REGULATOR_RTS5816_INIT_PRIORITY,        \
			 &regulator_rtsfp_api);
