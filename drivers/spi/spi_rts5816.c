...
#define DT_DRV_COMPAT realtek_rts5816_spi

#define LOG_LEVEL CONFIG_SPI_LOG_LEVEL
#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(spi_rts5816);
...
#include "spi_rts5816_m_ctrl_reg.h"
#include "spi_rts5816_dw_reg.h"
#include "spi_rts5816.h"
...
static void dw_reg_write(const struct spi_rts5816_config *cfg, uint32_t offset, uint32_t value)
{
	sys_write32(value, cfg->dw_regs + offset);
}

static uint32_t dw_reg_read(const struct spi_rts5816_config *cfg, uint32_t offset)
{
	return sys_read32(cfg->dw_regs + offset);
}

static void dw_reg_set_bits(const struct spi_rts5816_config *cfg, uint32_t offset, uint32_t bits)
{
	sys_set_bits(cfg->dw_regs + offset, bits);
}

static void dw_reg_clear_bits(const struct spi_rts5816_config *cfg, uint32_t offset, uint32_t bits)
{
	sys_clear_bits(cfg->dw_regs + offset, bits);
}

static void ctrl_reg_write(const struct spi_rts5816_config *cfg, uint32_t offset, uint32_t value)
{
	sys_write32(value, cfg->ctrl_regs + offset);
}

static uint32_t ctrl_reg_read(const struct spi_rts5816_config *cfg, uint32_t offset)
{
	return sys_read32(cfg->ctrl_regs + offset);
}

static void ctrl_reg_set_bits(const struct spi_rts5816_config *cfg, uint32_t offset, uint32_t bits)
{
	sys_set_bits(cfg->ctrl_regs + offset, bits);
}

static void ctrl_reg_clear_bits(const struct spi_rts5816_config *cfg, uint32_t offset,
				uint32_t bits)
{
	sys_clear_bits(cfg->ctrl_regs + offset, bits);
}

static inline bool spi_rts5816_is_slave(struct spi_rts5816_data *data)
{
	return (IS_ENABLED(CONFIG_SPI_SLAVE) && spi_context_is_slave(&data->ctx));
}

static inline bool spi_rts5816_transfer_ongoing(struct spi_rts5816_data *data)
{
...
{
	const struct spi_rts5816_config *cfg = dev->config;
	struct spi_rts5816_data *data = dev->data;
	...
	if (spi_rts5816_transfer_ongoing(data)) {
		return;
	}

out:
	if (!spi_rts5816_is_slave(data)) {
		/* disable cs ser */
		...
		/* Disable interrupts */
		ctrl_reg_clear_bits(cfg, R_MST_SPI_SSI_IRQ_ENABLE, MST_DONE_INT_ENABLE);
	} else {
...
static int spi_rts5816_configure(const struct device *dev, const struct spi_config *config)
{
	...
	if (cfg->max_xfer_size < SPI_WORD_SIZE_GET(config->operation)) {
		LOG_ERR("Max xfer size is %u, word size of %u not allowed", cfg->max_xfer_size,
...
static int spi_rts5816_transceive_once(const struct spi_rts5816_config *cfg,
				       struct spi_rts5816_data *data)
{
	size_t dma_len = 0;
	uint8_t *tx_buf = NULL;
	...
	if (spi_rts5816_is_slave(data)) {
		...
		if (!WAIT_FOR((ctrl_reg_read(cfg, R_MST_SPI_SSI_START_CTRL) & MST_SSI_START_MASK) ==
				      0,
			      RTS5816_SPI_TIME_OUT_MS * 1000, k_yield())) {
...
static int spi_rts5816_transceive_next(const struct spi_rts5816_config *cfg,
				       struct spi_rts5816_data *data)
{
	size_t dma_len = 0;
	uint8_t *tx_buf = NULL;
		      ...
		      bool asynchronous, spi_callback_t cb, void *userdata)
{
	...
	/* Configure */
	err = spi_rts5816_configure(dev, config);
	if (err) {
		goto out;
	}
	...
	if (cfg->interrupt_enable) {
		if (!spi_rts5816_is_slave(data)) {
			...
			ctrl_reg_set_bits(cfg, R_MST_SPI_SSI_IRQ_STATUS, MST_DONE_INT);
			/* Enable interrupt */
			ctrl_reg_set_bits(cfg, R_MST_SPI_SSI_IRQ_ENABLE, MST_DONE_INT_ENABLE);
		} else {
			/* TODO: spi slave support */
		}
		...
		err = spi_rts5816_transceive_once(cfg, data);
		if (err) {
			dw_reg_clear_bits(cfg, R_SSI_SSIENR, SSI_EN_MASK);
			ctrl_reg_clear_bits(cfg, R_MST_SPI_SSI_IRQ_ENABLE, MST_DONE_INT_ENABLE);
			goto out;
		...
		do {
			err = spi_rts5816_transceive_once(cfg, data);
		} while (!err && spi_rts5816_transfer_ongoing(data));
...
static int spi_rts5816_transceive(const struct device *dev, const struct spi_config *config,
...
static int spi_rts5816_transceive_async(const struct device *dev, const struct spi_config *config,
					const struct spi_buf_set *tx_bufs,
...
static int spi_rts5816_release(const struct device *dev, const struct spi_config *config)
{
	struct spi_rts5816_data *data = dev->data;
...
void spi_rts5816_isr(const struct device *dev)
{
	const struct spi_rts5816_config *cfg = dev->config;
	struct spi_rts5816_data *data = dev->data;
	...
	if (int_status & MST_DONE_INT_MASK) {
		...
		err = spi_rts5816_transceive_next(cfg, data);
	} else {
		err = -EIO;
	}
...
static int spi_rts5816_clock_enable(const struct device *dev)
{
	const struct spi_rts5816_config *cfg = dev->config;
	struct spi_rts5816_data *data = dev->data;
	int err = 0;
...
static const struct spi_driver_api rts5816_spi_api = {
	.transceive = spi_rts5816_transceive,
#ifdef CONFIG_SPI_ASYNC
	.transceive_async = spi_rts5816_transceive_async,
#endif /* CONFIG_SPI_ASYNC */
	.release = spi_rts5816_release,
};

int spi_rts5816_init(const struct device *dev)
{
	const struct spi_rts5816_config *cfg = dev->config;
	struct spi_rts5816_data *data = dev->data;
	int err = 0;
	...
	err = spi_rts5816_clock_enable(dev);
	if (err < 0) {
		return err;
	}
	...
	LOG_DBG("RTS5816 SPI driver initialized on device: %p", dev);
...
#define SPI_RTS5816_IRQ_HANDLER(inst)                                                              \
	static void spi_rts5816_irq_config_##inst(void)                                            \
	{                                                                                          \
		IF_ENABLED(DT_INST_PROP(inst, interrupt_enable),                         \
			   (IRQ_CONNECT(DT_INST_IRQN(inst), DT_INST_IRQ(inst, priority), \
					spi_rts5816_isr, DEVICE_DT_INST_GET(inst), 0);   \
			    irq_enable(DT_INST_IRQN(inst));))         \
	}

#define SPI_RTS5816_INIT(inst)                                                                     \
	IF_ENABLED(CONFIG_PINCTRL,               \
		  (PINCTRL_DT_INST_DEFINE(inst);))                                                 \
	SPI_RTS5816_IRQ_HANDLER(inst);                                                             \
	static struct spi_rts5816_data spi_rts5816_data_##inst = {                                 \
		SPI_CONTEXT_INIT_LOCK(spi_rts5816_data_##inst, ctx),                               \
		SPI_CONTEXT_INIT_SYNC(spi_rts5816_data_##inst, ctx),                               \
		SPI_CONTEXT_CS_GPIOS_INITIALIZE(DT_DRV_INST(inst), ctx)};                          \
	static const struct spi_rts5816_config spi_rts5816_config_##inst = {                       \
		.ctrl_regs = DT_INST_REG_ADDR_BY_NAME(inst, ctrl),                                 \
		.dw_regs = DT_INST_REG_ADDR_BY_NAME(inst, dw),                                     \
		.clock_dev = DEVICE_DT_GET(DT_INST_CLOCKS_CTLR(inst)),                             \
		.clock_subsys = (clock_control_subsys_t)DT_INST_PHA(inst, clocks, clkid),          \
		.irq_config_func = spi_rts5816_irq_config_##inst,                                  \
	...
	DEVICE_DT_INST_DEFINE(inst, spi_rts5816_init, NULL, &spi_rts5816_data_##inst,              \
			      &spi_rts5816_config_##inst, POST_KERNEL, CONFIG_SPI_INIT_PRIORITY,   \
			      &rts5816_spi_api);

DT_INST_FOREACH_STATUS_OKAY(SPI_RTS5816_INIT)
