...
static int spi_rts5817_transceive_once(const struct spi_rts5817_config *cfg,
		      ...
		      bool asynchronous, spi_callback_t cb, void *userdata)
{
	...
	if (cfg->interrupt_enable) {
		...
		err = spi_rts5817_transceive_once(cfg, data);
		if (err) {
			dw_reg_clear_bits(cfg, R_SSI_SSIENR, SSI_EN_MASK);
			ctrl_reg_clear_bits(cfg, R_MST_SPI_SSI_IRQ_ENABLE, MST_DONE_INT_ENABLE);
			goto out;
...
static const struct spi_driver_api rts5817_spi_api = {
