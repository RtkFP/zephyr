...
static bool rts_usb_test_hs(const struct device *dev)
{
	const struct udc_rts_config *cfg = dev->config;
	mem_addr_t ep_regs = cfg->u2sie_sys_base;

	return (((sys_read32(ep_regs + R_U2SIE_SYS_CTRL) & MODE_HS)) == MODE_HS);
}
...
static void rts_ctrl_handle_recv(const struct device *dev)
{
	...
	transfer_length = MIN(net_buf_tailroom(buf), transfer_length);
...
{
	return rts_usb_test_hs(dev) ? UDC_BUS_SPEED_HS : UDC_BUS_SPEED_FS;
}
